...
注意：外层用 `~~~` 围栏正是为了容纳内层的 ```bash 围栏——`~~~markdown` 与 `~~~` 之间是
`知识库/操作日志.md` 的**完整目标内容**，原样写入即可，内层围栏保持 ```bash 不变。
...
**同时核对全部索引条目与其 `tldr` 逐字一致。** CLAUDE.md 第五章规定 `tldr` **同时用作 `索引.md`
中该页的条目文字**，两份文字必须一致。Task 4 的修复轮已把 `摘要/清结算模块设计`、`摘要/支付系统技术决策`、
`摘要/跨库 Join 方案` 三条同步为新的 tldr（该分叉由 Task 4 的简报造成，已在修复轮 1 闭合）。
本步只需**复核**：逐条比对 `索引.md` 里每一行与对应页面 frontmatter 的 `tldr`，有出入就改正。
新加的两行自然带上新页面的 `tldr`。
...
- [ ] **Step 0: 给 `lint.md` 补上统一的「非页面文件」豁免**
...
**（a）「孤儿页」**现在是「无任何入链的页面（`索引.md`、`操作日志.md`、`综述.md` 除外）」。
`lint-report-*.md` 不被任何页面入链，会在**每次 lint 都把上一轮的报告报成孤儿**。改为：

> - [ ] **孤儿页**：无任何入链的页面（非页面文件除外）

**（b）「索引同步」**现在写的是「`知识库/` 下所有 `.md` 是否都在 `索引.md` 中列出」。照字面执行会把
`操作日志.md` 自己、`综述.md` 和每轮生成的 `lint-report-*.md` 都算作"漏登记"。改为：

> - [ ] **索引同步**：`知识库/{摘要,概念,实体,综合,输出}/` 下的页面是否都在 `索引.md` 中列出

**（c）「必填字段」**现在写的是「每页是否有 `title`、`tldr`…」。CLAUDE.md 第五章把六个必填字段的
约束加在**页面**上，并明确 `索引.md`／`操作日志.md`／`综述.md` 用轻 frontmatter 或不加。改为：

> - [ ] **必填字段**：每个页面是否有 `title`、`tldr`、`type`、`status`、`created`、`updated`（非页面文件除外）

**（d）「type 合法」与「status 合法」**同样只说「是否为…之一」，没有豁免。非页面文件没有 `type`，
照字面执行会把它们全打成非法。两项各补「（非页面文件除外）」。

**（e）断链检查只额外跳过 `lint-report-*.md`**：报告是生成产物，正文里会引用链接举例，
下一轮扫描会把这些举例当成真链接。**注意这里不能照搬「非页面文件」那一组**——
`索引.md` 集中了全库最该验的条目链接，`操作日志.md` 与 `综述.md` 也各有真链接，
把它们一并跳过等于把断链检查废掉大半。Step 1 的脚本只加 `lint-report-*.md` 一行跳过。

> 为什么必须现在修：Task 6 **就是第一次真跑 lint**，上述五项会在这一轮之后全部撞上——
> 本轮生成的报告，会被下一轮 lint 报成「孤儿 + 缺字段 + type 非法 + status 非法」。
...
for p in glob.glob('知识库/**/*.md', recursive=True):
    # 只跳过 lint 报告：它是生成产物，正文里可能引用链接举例。
    # 索引.md / 操作日志.md / 综述.md **不跳过**——它们的链接恰恰是最该验的。
    if fnmatch.fnmatch(os.path.basename(p), 'lint-report-*.md'): continue
...
**本文件是生成产物，不加 frontmatter**（与 Step 0 把它列入「非页面文件」豁免的事实一致）。
...
汇报内容：本次产出清单、裁决 A–D 的落实情况、lint 结果、**仍需用户在 Obsidian 内验证的运行时项**（若上一阶段的每日笔记插件验证仍未做，一并提示）、以及阶段 4（其余 18 篇剪藏全量编译）的批次建议。
...
**其余为 18 篇**而非原文所写的 20 篇。（该处出现在 Task 6 的 Interfaces 行与 Step 8 两处；
Interfaces 行随 修订 2 一并改掉，Step 8 的措辞在 修订 3 补齐——原文只改了一处。）

### 修订 3（2026-09-18，Task 4 审查后）

Task 4 的审查判 ❌，1 条 Important。根因是**本计划自造的约束冲突**：Task 4 同时要求
「`tldr` ≤60 字」与「Files 里不含 `索引.md`」，于是任何超过 60 字的索引条目被压缩后必然与索引分叉。

| # | 问题 | 修法 |
|---|---|---|
| 6 | `tldr` 与 `索引.md` 条目文字在三页上不一致，违反 CLAUDE.md 第五章的「tldr 同时用作索引条目文字」不变式 | **并入 Task 4 的修复轮 1**（不在 Task 5 重做）：把三条索引条目的文字替换为各自页面 frontmatter 的新 `tldr`，逐字复制。Task 5 Step 4 改为**复核**全部索引条目与 tldr 一致，而非执行同步 |
| 7 | `confidence: high` 出现在 CLAUDE.md 的 `type: source` 模板里，但四个摘要页都没有该字段，它也不在六个必填字段内；第五章的 `confidence` 判定标准另写在概念页/实体页节下 | **并入 Task 4 的修复轮 1**：从 source 模板删掉 `confidence`，并补一句「`confidence` 只用于 concept／entity／synthesis／output；摘要页是来源的直接派生，不填」 |

裁决理由：`confidence` 表达的是**跨来源判断的可靠度**，摘要页是单一来源的直接派生，填它没有信息量。
文章原文的 lint 必填字段里也没有 `confidence`。schema 本身没有争议，故不需要上报用户拍板。

### 修订 4（2026-09-18，Task 5 完成后、Task 6 派发前）

修订 2 只修了「索引同步」与「必填字段」两项，**没意识到这是同族检查各自写各的豁免清单**。逐项
核对 `lint.md` 全文后发现，还有三项会被本轮生成的 `lint-report-2026-09-18.md` 撞上：
「孤儿页」的豁免清单只有三个文件、「type 合法」与「status 合法」没有任何豁免。结果是——
**Task 6 这一轮生成的报告，会被 Task 6 之后的下一轮 lint 报成「孤儿 + 缺字段 + type 非法 + status 非法」**。

修法不是再打三个补丁，而是先把「非页面文件」定义一次，再让三项检查共同引用（Step 0 新增 0 项），
并顺手把「孤儿页」原有清单也统一过去：

| # | 问题 | 修法 |
|---|---|---|
| 8 | 「孤儿页」豁免清单只有 `索引.md`／`操作日志.md`／`综述.md`，未含 `lint-report-*.md` | Step 0(a)：改为引用统一的「非页面文件」 |
| 9 | 「type 合法」「status 合法」无任何豁免，非页面文件会被全数判非法 | Step 0(d)：两项各补「（非页面文件除外）」 |
| 10 | 断链脚本会扫描上一轮报告里的链接举例 | Step 0(e) + Step 1 脚本：**只**额外跳过 `lint-report-*.md` |

**刻意不做的**：断链检查**不**套用「非页面文件」那一组豁免。`索引.md` 集中了全库最该验的条目链接，
把它跳过等于废掉断链检查的大半——豁免的边界必须按**检查性质**划，不能按文件名一刀切。

同时约定：`知识库/lint-report-YYYY-MM-DD.md` 为生成产物，**不加 frontmatter**，与它被三项检查
豁免的事实保持一致。