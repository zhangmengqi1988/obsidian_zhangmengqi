...
**同时把 Task 4 精简过 tldr 的三条索引条目同步为新的 tldr 文字。** Task 4 为满足 ≤60 字约束，对
`摘要/清结算模块设计`、`摘要/支付系统技术决策`、`摘要/跨库 Join 方案` 三页的 tldr 做了精简，
但 `索引.md` 里仍是原来的长描述。CLAUDE.md 第五章规定 `tldr` **同时用作 `索引.md` 中该页的条目文字**，
两份文字必须一致，否则 schema 与产物当场打架。改法：把这三条的条目文字替换为各自页面 frontmatter 里
的新 `tldr` 值（逐字复制，不要另写一版）。其余条目已是 ≤60 字，不动。
